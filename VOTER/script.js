...
function verify()
{
    document.getElementById("walletsubmit").style.display = "none";
    var input = document.createElement('input');
    var button = document.createElement('button');
    input.type = 'text';
    input.id='vst2';
    input.placeholder='Mention your aadhar registered contact no.';
button.type = 'text';
...
button.onclick = function send() {
    console.log(document.getElementById("vst2").value);
  var b;
...
{
   console.log(tx);
    contract.methods.checkVoter(document.getElementById("vst2").value).call({from:tx}, function(error, result){
   console.log(result);
   if(result){
let mobileNumber = Number(document.getElementById("vst2").value);
...
t1=false;
document.getElementById("box3").innerHTML="Not verified❌";
}
...
document.getElementById("box3").appendChild(input);
document.getElementById("box3").appendChild(button);
var verified;

function t5(){
    document.getElementById("box3").removeChild(input);
document.getElementById("box3").removeChild(button);
...
button1.id = 'btn2';
document.getElementById("box3").appendChild(input1);
document.getElementById("box3").appendChild(button1);
button1.onclick=function verify1()
    {
        ...
        .then((data)=>{
            if(data["Details"] == "OTP Matched")
            {
                ...
                {
                    var acc = accounts[0];
                    return contract.methods.reviewVoter1(acc,true).send({from:acc});
                }).then(function(tx)
                {
                    console.log(tx);
                ...
                console.log(data);
                alert("OTP Matched");
                document.getElementById("box3").removeChild(input1);
                document.getElementById("box3").removeChild(button1);
                document.getElementById("box3").innerHTML="Verified✅";
            }).catch(function(tx)
            {
                console.log(tx);
            })
...
document.getElementById("box2").innerHTML="Not verified❌";
    ...
    if (timer) {
        ...
        document.getElementById("timer").innerHTML = "Timer Expires within "+Number(10-parseInt(secString))+" seconds";
        }
        else{
